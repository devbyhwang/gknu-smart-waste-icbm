...
def test_outputm_refreshes_four_sensor_levels_without_classification():
    class SensorDisplay:
        def __init__(self):
            self.calls = []

        def showSensorStatus(self, fill_levels=None, full_bins=None):
            self.calls.append((fill_levels, full_bins))
    ...
    output = OutputM()
    output.display = SensorDisplay()
    output.sensor = FillSensor(0.0)
    output.sensors = {
        WasteType.CAN: FillSensor(0.10),
        WasteType.PLASTIC: FillSensor(0.45),
        WasteType.GLASS: FillSensor(0.81),
        WasteType.PAPER: FillSensor(0.95),
    }

    fill_levels, full_bins = output.refreshSensorDisplay()

    assert fill_levels == {
        WasteType.CAN: 0.10,
        WasteType.PLASTIC: 0.45,
        WasteType.GLASS: 0.81,
        WasteType.PAPER: 0.95,
    }
    assert full_bins == {WasteType.GLASS, WasteType.PAPER}
    assert output.display.calls == [(fill_levels, full_bins)]


def test_outputm_maps_legacy_multi_ultrasonic_sensor_lists_to_bins():
    class MultiSensor:
        fillThreshold = 0.8

        def checkFillLevel(self):
            return [0.05, 0.3, 0.85, 1.2]

        def isFull(self):
            return [False, False, True, True]

    output = OutputM()
    output.sensor = MultiSensor()
    output.sensors = {}

    fill_levels = output.collectFillLevels()
    full_bins = output.collectFullBins(fill_levels)

    assert fill_levels == {
        WasteType.CAN: 0.05,
        WasteType.PLASTIC: 0.3,
        WasteType.GLASS: 0.85,
        WasteType.PAPER: 1.0,
    }
    assert full_bins == {WasteType.GLASS, WasteType.PAPER}
    assert output.checkBinFull() is True
